//
//  PromoteSmartMem2Reg.cpp
//  SmartSSA
//
//  Created by Abu Naser Masud on 2021-09-29.
//

//#include "PromoteSmartMem2Reg.hpp"
...
public:
  SmartMem2Reg(ArrayRef<AllocaInst *> Allocas, DominatorTree &DT,
                 AssumptionCache *AC, bool LA, bool DumpCFG,bool startDef)
    : Allocas(Allocas.begin(), Allocas.end()), DT(DT), F(*DT.getRoot()->getParent()),
        DIB(*DT.getRoot()->getParent()->getParent(), /*AllowUnresolved*/ false),
        AC(AC), SQ(DT.getRoot()->getParent()->getParent()->getDataLayout(),
                   nullptr, &DT, AC), liveness(LA), viewCFG(DumpCFG), startNodeDef(startDef) {}
    
    
    SmartMem2Reg(ArrayRef<AllocaInst *> Allocas, Function &Func, DominatorTree &DT,
                   AssumptionCache *AC,bool LA,bool DumpCFG,bool startDef)
        : Allocas(Allocas.begin(), Allocas.end()), DT(DT),F(*DT.getRoot()->getParent()),
          DIB(*DT.getRoot()->getParent()->getParent(), /*AllowUnresolved*/ false),
          AC(AC), SQ(DT.getRoot()->getParent()->getParent()->getDataLayout(),
  ...
  void run(std::vector<std::pair<std::set<unsigned>,BitVector>> *PhiBlocks);
  void runOriginal(std::vector<std::pair<std::set<unsigned>,BitVector>> *PhiBlocks);
...
void SmartMem2Reg::run(std::vector<std::pair<std::set<unsigned>,BitVector>> *PhiNodes) {
  ...
  for (unsigned AllocaNum = 0; AllocaNum != Allocas.size(); ++AllocaNum) {
   ...
   //   else Info.DefiningBlocks.emplace_back(&F.getEntryBlock());
    ...
    if(startNodeDef)
       Info.DefiningBlocks.push_back(&F.getEntryBlock());
      
    // Determine which blocks the value is live in.  These are blocks which lead
    // to uses.
    SmallPtrSet<BasicBlock *, 32> LiveInBlocks;
    if(liveness) ComputeLiveInBlocks(AI, Info, DefBlocks, LiveInBlocks);
    ...
    CDSSA *ssa=new CDSSA(F,BBNumbers,Info.DefiningBlocks,Info.UsingBlocks, LiveInBlocks,true,liveness);
     ...
     // PhiNodes.push_back(std::set<unsigned>());
      if(PhiNodes){
          ...
          std::set<unsigned> X;
          X=ssa->getSSA();
          for(auto B:Info.DefiningBlocks)
              DefVect[BBNumbers[B]]=true;
          PhiNodes->push_back(std::pair<std::set<unsigned>,llvm::BitVector>(X,DefVect));
      }
...
void SmartMem2Reg::runOriginal(std::vector<std::pair<std::set<unsigned>,llvm::BitVector>> *PhiNodes) {
  Function &F = *DT.getRoot()->getParent();
  AllocaDbgDeclares.resize(Allocas.size());
  ...
  for (unsigned AllocaNum = 0; AllocaNum != Allocas.size(); ++AllocaNum) {
    ...
    if(PhiNodes){
        ...
        std::set<unsigned> Ps;
        for(auto B:PHIBlocks)
            Ps.insert(BBNumbers[B]);
        PhiNodes->push_back(std::pair<std::set<unsigned>,llvm::BitVector>(Ps,DefVect));
    }
    llvm::sort(PHIBlocks, [this](BasicBlock *A, BasicBlock *B) {
      return BBNumbers.find(A)->second < BBNumbers.find(B)->second;
    });
...
void SmartMem2Reg::ComputeLiveInBlocks(
...
bool SmartMem2Reg::QueuePhiNode(BasicBlock *BB, unsigned AllocaNo,
                                  unsigned &Version) {
...
void SmartMem2Reg::RenamePass(BasicBlock *BB, BasicBlock *Pred,
...
void llvm::PromoteSmartMemToReg(ArrayRef<AllocaInst *> Allocas, Function &F, DominatorTree &DT, AssumptionCache *AC, bool liveness,std::vector<std::pair<std::set<unsigned>,llvm::BitVector>> *ssadual, bool dumpCFG,bool startDef) {
  // If there is nothing to do, bail out...
  if (Allocas.empty())
    return;
  SmartMem2Reg(Allocas,F, DT, AC,liveness,dumpCFG,startDef).run(ssadual);
}

// mem2reg Algorithm
void llvm::PromoteOriginalMemToReg(ArrayRef<AllocaInst *> Allocas, DominatorTree &DT,
                           AssumptionCache *AC, bool liveness,std::vector<std::pair<std::set<unsigned>,llvm::BitVector>> *ssam2r, bool dumpCFG,bool startDef) {
  if (Allocas.empty())
    return;
  SmartMem2Reg(Allocas, DT, AC,liveness,dumpCFG,startDef).runOriginal(ssam2r);
